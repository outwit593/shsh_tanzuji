<template>
  ...
  <div class="fuels-section" :style="fuelsSectionStyle">
  <div class="fuels-row">
    <div class="fuels-container" :style="fuelsContainerStyle">
      <div class="fuel-container">
        <div v-for="fuel in fuels" :key="fuel.productName" v-if="fuel.production > 0" class="fuel">
          <h4>{{ fuel.productName }}</h4>
          <img :src="getImagePath()" alt="Fuel Image" class="fuel-image" />
          <h4>碳排放量: {{ fuels[0].carbonFootprint }} tCO<sub>2</sub>e/t</h4>
          <h4>产量: {{ fuel.production.toFixed(2) }} 吨</h4>
        </div>
      </div>
      <div class="info-boxes" :style="infoBoxesStyle">
        <div class="info-box">
          <div class="info-title">本月总碳排（吨二氧化碳）</div>
          <div class="info-value">{{ totalCarbonEmission.toFixed(2) }} 吨</div>
        </div>
        <div class="info-box">
          <div class="info-title">本月生产燃料油产品总量</div>
  ...
  export default {
    data() {
      return {
        ...
        fuellProducts: ['低硫燃料油', 'RMG180船用燃料油(Ⅱ)', 'RMG380CST', 'LSMGO', '轻质燃料油', '燃料油组分', '5号低硫工业燃料油', '低硫船用燃料油'],
        years: ['2022', '2023'],
...
'燃料油调合装置': '#BAB0AC'  // 灰色
    ...
    // computed: {
    //   filteredFuels() {
    //     return this.fuels.filter(fuel =>
    //       ['低硫燃料油', 'RMG180船用燃料油(Ⅱ)', 'RMG380CST', 'LSMGO', '轻质燃料油', '燃料油组分', '5号低硫工业燃料油', '低硫船用燃料油'].includes(fuel.name)
    //     );
    ...
    fuelsContainerStyle() {
      if (this.fuels.length === 1) {
        return { marginLeft: '-1000px' };
      } else if (this.fuels.length === 2) {
        return { marginLeft: '-1000px' };
      } else if (this.fuels.length === 3) {
        return { marginLeft: '-1100px' };
      } else if (this.fuels.length >= 4) {
        return { marginLeft: '-1000px' };
      }
    },
    infoBoxesStyle() {
    ...
    getImagePath() {
        ...
        try {
          ...
          const folderName = `${year}fuel-image`; // 根据年份选择文件夹
  ...
  try {
    const yearMonth = dayjs(this.selectedDate).format('YYYY-MM');
    const response = await axios.get(`https://172.160.11.205:443/ranliaoyoucarbonlabel-energy-data`, {
      params: {
        type: 'ranliaoyoucarbonlabel',
    ...
    if (response.status === 200) {
      this.fuels = response.data || []; // 确保数据有效
    } else {
    ...
    this.fuels = []; // 设置为空数组，避免undefined错误
  }
},
      ...
      async fetchData() {
        try {
          const [consumptionResponse, emissionsResponse, devices_emissionsResponse, departmentDataResponse, trendDataResponse, productOutputResponse] = await Promise.all([
            axios.get(`https://172.160.11.205:443/ranliaoyoucarbonlabel-energy-data`, {
            // axios.get(`/ranliaoyoucarbonlabel-energy-data`, {
              params: {
                type: 'consumption',
                year: this.selectedYear,
                month: this.selectedMonth
  ...
  // 根据数据生成每个月的产品产量
  const productNames = [
   '低硫燃料油', 'RMG180船用燃料油(Ⅱ)', 'RMG380CST', 'LSMGO', '低硫船用燃料油', '轻质燃料油', '燃料油组分', '5号低硫工业燃料油'
  ]; // 产品名称列表

  const colors = [
    '#4a90e2', '#e94e77', '#9ab1ef', '#2de038', '#686de0',
    '#d97a5c', '#6e9ef1', '#e5cf0d'
  ]; // 为每个产品定义一种颜色
...
fetchDevicesEmissionsData() {
  ...
  const currentMonth = this.selectedMonth;
      ...
      axios.get(`https://172.160.11.205:443/ranliaoyoucarbonlabel-energy-data`, { params: {
                type: 'devices_carbon_emissions',
                year: this.selectedYear,
                month: this.selectedMonth
              } })
...
.trend-chart-container {
   ...
   */
  .app-container, .select-wrapper, .fuel {
    font-family: '宋体', SimSun, sans-serif;
...
.fuels-container {
    ...
    gap: 20px;
    padding: 20px;
    max-width: 1000px; /* 设置一个足够宽的最大宽度以容纳三个图片 */
    position: absolute; /* 绝对定位相对于 fuels-section */
  top: 50%; /* 垂直居中 */
  left: 50%; /* 水平居中 */
...
.info-boxes {
  ...
  min-width: 200px; /* 或者你希望的任何最小宽度 */
  margin-left: 1000px; /* 设置info-boxes左边的边距 */
  margin-top: -615px;
}
